...
from gtochess.domain.repertoire import SelectionPolicy
from gtochess.layout import CONTAINER_ENGINE_DIR, engine_binary_name, local_engine_dir
...
class Settings(BaseSettings):
    model_config = SettingsConfigDict(env_prefix="GTOCHESS_", env_file=".env", extra="ignore")
    ...
    lichess_client_id: str = "gtochess.local"
    ...
    # Set to move every store off the shared filesystem and into object storage.
    ...
    def resolve_engine_path(self) -> Path:
        if self.engine_path is not None:
            if not self.engine_path.exists():
                raise EngineNotProvisioned(
                    f"GTOCHESS_ENGINE_PATH does not exist: {self.engine_path}"
                )
        ...
        raise EngineNotProvisioned(
            ...
            "For local development run:  python -m gtochess.tools.fetch_stockfish\n\n"
