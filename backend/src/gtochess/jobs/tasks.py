...
from gtochess.jobs.app import app
from gtochess.jobs.notify import deliver
from gtochess.storage import get_storage

IMPORT_TASK = "gtochess.import_player"
ANNOTATE_TASK = "gtochess.annotate_player"
LEARN_TASK = "gtochess.learn_positions"
MEASURE_TASK = "gtochess.measure_losses"
...
def learn_positions(
    ...
    from gtochess.analysis.knowledge import learn_position
    from gtochess.config import EngineNotProvisioned
    ...
    from gtochess.llm.explain import study_position
    ...
    games, _ = load_player_games(username, get_storage())
    if not games:
...
def measure_player_losses(
    ...
    from gtochess.analysis.sweep import SweepProgress, plan_sweep, run_sweep
    from gtochess.config import EngineNotProvisioned
    from gtochess.domain.games import Side
    from gtochess.ingest.graph import build_graph
...
def annotate_player(
    ...
    games, stamp = load_player_games(username, get_storage())
    if not games:
        return {"username": username, "failed": f"no games imported for {username!r} yet"}
    ...
    AnnotationStore(get_storage()).write(result)
    body = {
        "username": username,
        "shape": shape,
