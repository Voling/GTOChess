...
data "aws_iam_policy_document" "task" {
  ...
  dynamic "statement" {
    for_each = var.enable_object_storage ? [1] : []
    content {
  ...
  # head_object on a key that is not there answers 403 rather than 404 without
  # this, so "does the import exist" would read as a permissions failure.
  dynamic "statement" {
    for_each = var.enable_object_storage ? [1] : []
    content {
