...
class PositionKey(BaseModel):
    ...
    def cache_path(self, pipeline_version: str) -> str:
        """Content-addressed S3/CDN key. Immutable: bump the version, never purge."""
        return f"explanations/{self.variant.value}/{self.digest}/{pipeline_version}.json"
...
class EvalLandscape(BaseModel):
    ...
    def is_single_answer(self) -> bool:
        ...
        if self.legal_move_count == 1:
            return True
        return self.delta_to_second_cp is not None and self.delta_to_second_cp >= 300
...
class Claim(BaseModel):
    """One assertion, bound to one piece of evidence.

    A claim whose ``evidence_id`` does not resolve is dropped before storage.
    That single rule is what kills both hallucination and generic filler --
    "develop your pieces and control the centre" has nothing to cite.
    """

    text: str = Field(max_length=400)
    move_refs: list[str] = Field(default_factory=list, description="SAN, from the supplied set")
    evidence: EvidenceKind
    evidence_id: str


class Explanation(BaseModel):
    """Structured LLM output. Never free prose."""

    headline: str = Field(max_length=200, description="Leads with the top-sensitivity element")
    claims: list[Claim] = Field(min_length=1, max_length=8)
    analogy: str | None = Field(
        default=None, description="Relation to a supplied precedent position, if any"
    )
    disanalogy: str | None = Field(
        default=None, description="Where the precedent breaks down -- often the best content"
    )
...
class RejectionReason(StrEnum):
    ...
    CONTRADICTS_ENGINE = "contradicts_engine"
    MISSED_SENSITIVITY = "missed_sensitivity"
    PLATITUDE = "platitude"
    OFF_TIER = "off_tier"
...
class ValidationResult(BaseModel):
    ...
    surviving: Explanation | None = None
