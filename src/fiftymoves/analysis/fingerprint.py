...
def fingerprint_from_pv(
    ...
    token_string = " ".join(s.token() for s in steps)
    digest = hashlib.blake2b(token_string.encode(), digest_size=12).hexdigest()
    return PlanFingerprint(steps=steps, digest=digest)
